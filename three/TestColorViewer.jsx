...
export default class TestColorViewer extends Component {
    ...
    onClickSwitchColor = (event) => {
        ...
        if( this.state.vertexColors ){
            this.root.current.appendChild(this.mapper.labelRenderer.domElement)
            const controlsLabel = new OrbitControls( this.viewer.perpCamera, this.labelRenderer.domElement );
            controlsLabel.minDistance = 5;
            controlsLabel.maxDistance = 100;
            controlsLabel.addEventListener('change', this.mapper.renderScene)
            this.mapper.mapOn()
        } else {
            this.mapper.mapOff()
        }
